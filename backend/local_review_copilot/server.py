...
def update_runtime_config(request: UpdateConfigRequest) -> dict[str, Any]:
    ...
    if request.workspace_root_dir is not None:
        config.workspace.root_dir = request.workspace_root_dir

    if request.llm is not None:
        config.llm = LLMConfig(**_dump_model(request.llm))
        llm_client = LLMClient(config.llm)
...
async def review(request: ReviewRequest) -> dict[str, Any]:
    ...
    storage.save_session(session)
    return {"trace_id": trace_id, "report": _dump_model(report), "exports": exports}
...
async def quiz_generate(request: QuizGenerateRequest) -> dict[str, Any]:
    ...
    storage.save_session(session)
    return {"trace_id": trace_id, "items": [_dump_model(item) for item in quiz_items]}
